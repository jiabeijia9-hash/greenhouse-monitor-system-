...
u16 threshold_high;     //高温阈值(get_temperature单位), 默认900=50.0°C
u16 threshold_low;      //低温阈值, 默认0=-40.0°C
u8  rx_buffer[16];      //串口接收缓冲区
u8  rx_index;           //接收索引
bit rx_complete;        //一帧接收完成标志
u8  led7_toggle_cnt;    //LED7闪烁计数器
...
void main(void)
{
    ...
    P4M1 &= ~0x80;  P4M0 |= 0x80;   //P4.7推挽输出(驱动LED7)
    UART1_Init();
    display_index = 0;
    threshold_high = 650;   //25.0°C (测试用, 室温即可触发)
    threshold_low  = 0;     //-40.0°C (等于最低温, 实际禁用)
    rx_index = 0;
    rx_complete = 0;
    led7_toggle_cnt = 0;
    P1ASF = 0x0C;       //P1.2 P1.3做ADC
    P1M1 |= 0x0C; P1M0 &= ~0x0C;   //P1.2 P1.3高阻输入(ADC必需)
        ...
        if(B_1ms)   //1ms到
        {
            ...
            if(++msecond >= 300)    //300ms到
            {
                ...
                */
                    ...
                    if(j >= 0xFFF0) //ADC错误，显示----
                    {
                        for(i=0; i<4; i++)  LED8[i+4] = DIS_;
                        P47 = 1;
                        led7_toggle_cnt = 0;
                    }
                    else
                    {
                    if(j >= threshold_high || j <= threshold_low)
                    {
                        if(++led7_toggle_cnt >= 2)
                        {
                            led7_toggle_cnt = 0;
                            P47 = !P47;
                        }
...

                    {
                        ...
                        temp_str[idx++] = 'T';
                        temp_str[idx++] = F0 ? '-' : '+';
                        temp_str[idx++] = (whole / 100) % 10 + '0';
                        temp_str[idx++] = (whole / 10) % 10 + '0';
                        temp_str[idx++] = whole % 10 + '0';
                        temp_str[idx++] = '.';
                        temp_str[idx++] = frac + '0';
                        temp_str[idx++] = '\r';
                        temp_str[idx++] = '\n';
...
void UART1_Init(void)
{
    ...
    TMOD = (TMOD & 0x0F) | 0x20;   // Timer1 mode 2 (8-bit auto-reload)
    ...
    if(RI)
    {
        ...
        if(rx_index < sizeof(rx_buffer) - 1)
        {
            ...
            if(dat == '\n')
            {
                ...
                if(rx_index >= 2 && rx_buffer[rx_index - 2] == '\r')
...
void ParseCommand(void)
{
    ...
    if(cmd == 'H') threshold_high = val;
    else           threshold_low  = val;
